# Load and evaluate the MFISNet_Parallel on the test set
...
from train_MFISNet_Parallel import load_data
...
def setup_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        type=float,
        help="Noise-to-signal ratio for input test data",
    )
    a = parser.parse_args()
    return a
...
def main(args: argparse.Namespace) -> None:
    ...
    #########################################################
    # Find the best epoch and load the model
    ...
    model = load_MFISNet_Parallel_from_state_dict(model_state_dict)
    ...
    # is_nan_target = torch.isnan(test_targets_arr[:, 0, 0])
    # is_nan_preds_sum = torch.sum(is_nan_preds)
    # is_nan_target_sum = torch.sum(is_nan_target)
    # assert torch.all(
    #     is_nan_preds == is_nan_target
    # ), f"{is_nan_preds_sum} vs {is_nan_target_sum} NaNs found"
