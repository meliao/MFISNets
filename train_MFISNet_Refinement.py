...
def setup_args(argument_string: str = None) -> argparse.Namespace:
    ...
    parser.add_argument("-n_epochs_finetune", type=int, default=10)
...
class MultiFreqResidData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = self.d_mh[idx]
        return x, self.q_polar_lpf[idx, self.freq_output_idx], self.q_polar_lpf[idx, -1]
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.n_epochs_pretrain:
        for i in range(final_training_part_int, N_freqs):
            training_part = f"pretrain_{i}"
            dset_train.freq_output_idx = i
            dset_val.freq_output_idx = i
            model.freq_pred_idx = i

            epoch_stagger = args.n_epochs_pretrain * i
            n_epochs_this_step = args.n_epochs_pretrain

            # Need to do some accounting to get the number of epochs right during the
            # pretraining part that was interrupted.
            if i == final_training_part_int and pre_existing_bool:
                epoch_stagger_old = epoch_stagger
                epoch_stagger = final_epoch + 1
                n_epochs_this_step = n_epochs_this_step - (
                    epoch_stagger - epoch_stagger_old
                )

            # SET ONLY THE Ith FYNET BLOCK TO BE TRAINABLE
            for j, block_j in enumerate(model.inverse_networks):
                if j == i:
                    for p in block_j.parameters():
                        p.requires_grad = True
                else:
                    for p in block_j.parameters():
                        p.requires_grad = False

            # Decrease the learning rate if desired
            if args.lr_decrease_factor is not None:
                lr_i = (args.lr_decrease_factor**i) * args.lr_init
                eta_min_i = (args.lr_decrease_factor**i) * args.eta_min
            else:
                lr_i = args.lr_init
            ...
            # Now we want to find the best weights of this part of training and load these weights
            # back into the model.
            ...
            best_weight_fp_i = os.path.join(
                args.model_weights_dir, f"epoch_{best_epoch}.pickle"
            )
            state_dict = torch.load(best_weight_fp_i, map_location=torch.device("cpu"))
            logging.debug("Now have state_dict with keys: %s", state_dict.keys())

            # Loop over the previous blocks, setting their weights
            for j in range(i + 1):
                key_str_j = f"inverse_networks.{j}."
                ll = len(key_str_j)
                state_dict_j = {
                    k[ll:]: v for k, v in state_dict.items() if k.startswith(key_str_j)
                }
                logging.debug(
                    "Now have state_dict_j with keys: %s", state_dict_j.keys()
                )
            ...
            if args.warm_start_init and i + 1 < N_freqs:
                ...
                ll = len(key_str_i)
                state_dict_i = {
                    k[ll:]: v for k, v in state_dict.items() if k.startswith(key_str_i)
                }
                logging.debug(
                    "Now have state_dict_i with keys: %s", state_dict_i.keys()
                )

                model.inverse_networks[i + 1].load_state_dict(state_dict_i)

            model = model.to(device)

    ##########################################################################################
    # FINE-TUNE THE WHOLE THING
    training_part = "fine_tune"
    loss_fn_dd = {
        "mse": loss_module.mse,
        "psnr": loss_module.psnr,
        "rel_l2": loss_module.relative_l2_error,
    ...
    # Set all of the blocks trainable
    for block_j in model.inverse_networks:
        for p in block_j.parameters():
            p.requires_grad = True

    model.fine_tune_bool = True

    model.freq_pred_idx = N_freqs - 1
    dset_train.freq_output_idx = N_freqs - 1
    dset_val.freq_output_idx = N_freqs - 1

    epoch_stagger = args.n_epochs_pretrain * N_freqs
    n_epochs_this_step = args.n_epochs_finetune

    # Need to do some accounting to get the number of epochs right during the
    # fine-tuning part that was interrupted.
    if training_part == final_training_part and pre_existing_bool:
        epoch_stagger_old = epoch_stagger
        epoch_stagger = final_epoch + 1
        n_epochs_this_step = n_epochs_this_step - (epoch_stagger - epoch_stagger_old)

    if args.lr_decrease_factor is not None:
        lr_i = (args.lr_decrease_factor**N_freqs) * args.lr_init
        eta_min_i = (args.lr_decrease_factor**N_freqs) * args.eta_min
    else:
        lr_i = args.lr_init
        eta_min_i = args.eta_min
    model = train(
